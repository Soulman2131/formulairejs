...
const pseudoCheck = (value)=>{
    if (value.length > 0 && (value.length < 3 || value.length > 20)) {
        errorDisplay("pseudo", "Le pseudo doit contenir au minimum 3, et au maximum 20 caract\xe9res ! ");
        pseudo = null;
    } else if (!value.match(/^[a-zA-Z0-9_.-]*$/)) {
        errorDisplay("pseudo", "Le pseudo ne doit pas contenir de caract\xe9res sp\xe9ciaux");
        pseudo = null;
    } else {
...
const passwordCheck = (value)=>{
    if (!value.match(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/)) {
        ...
        errorDisplay("password", "Minimum de 8 caract\xe9res, un majuscule, un miniscule et un caract\xe9re sp\xe9cial", password = null);
...
form.addEventListener("submit", (e)=>{
    e.preventDefault();
    if (!pseudo || !email || !password || !confirm) alert("Veuillez remplir correctement les champs");
    else {
        alert(`Bravo ${pseudo} !! Votre inscription est valid\xe9e..`);
        pseudo = "";
        email = "";
        password = "";
        confirm = "";
